...
const CalendarPage = () => {
    ...
    const statusConfig: Record<string, { dot: string; bg: string; text: string; label: string }> = {
        scheduled: { dot: 'bg-blue-500', bg: 'bg-blue-50 border-blue-200', text: 'text-blue-700', label: 'Scheduled' },
        in_progress: { dot: 'bg-amber-500', bg: 'bg-amber-50 border-amber-200', text: 'text-amber-700', label: 'In Progress' },
        completed: { dot: 'bg-emerald-500', bg: 'bg-emerald-50 border-emerald-200', text: 'text-emerald-700', label: 'Completed' },
        cancelled: { dot: 'bg-slate-400', bg: 'bg-slate-50 border-slate-200', text: 'text-slate-500', label: 'Cancelled' }
    };
    ...
    const upcomingJobs = useMemo(() => {
        ...
        return jobs
            .filter(j => j.date_scheduled && j.date_scheduled >= today && j.status !== 'cancelled' && j.status !== 'completed')
            .sort((a, b) => (a.date_scheduled || '').localeCompare(b.date_scheduled || ''));
    }, [jobs]);

    // Today's jobs
    const todayJobs = useMemo(() => {
        return jobs.filter(j => j.date_scheduled?.startsWith(todayStr) && j.status !== 'cancelled');
    }, [jobs, todayStr]);
        ...
        <div className="space-y-4 md:space-y-6">
            ...
            <div className={`md:hidden ${mobileView === 'calendar' ? 'block' : 'hidden'}`}>
                <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
                    {/* Month nav */}
                    <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-[#003875] to-delaval-blue">
            ...
            <div className="hidden md:grid lg:grid-cols-[1fr_340px] gap-6">
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                    {/* Header */}
                    <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-[#003875] to-delaval-blue">
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title="Create New Job">
                <form onSubmit={handleCreateJob} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        ...
                        <button type="submit" className="px-6 py-2.5 bg-gradient-to-br from-delaval-blue to-[#003875] text-white rounded-lg font-bold shadow-lg shadow-blue-900/20">Create Job</button>
                    </div>
                </form>
            </Modal>
        </div>
